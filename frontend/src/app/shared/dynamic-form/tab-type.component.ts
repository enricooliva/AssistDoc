...
<button type="button"    
    ...
    (click)="setStep(index)">{{ f.props.stepLabel ? f.props.stepLabel : 'btn_'+(field.key || '')+'_'+index | translate }}</button>
</ng-container>
</div>
<div class="btn-group" role="group">
...
export class TabTypeComponent extends FieldType implements OnInit {
  ...
  ngAfterViewInit() {  
    //per mettere il fuoco all'apertura sul primo elemento  
    this.setFocusOnFirstField(this.activedStep);  
  ...
  setFocusOnFirstField(index: number) {
    const group = this.field.fieldGroup[index];
    if (!group || !group.fieldGroup) {
      return;
    }

    const firstField = this.findFirstFieldWithoutGroup(group.fieldGroup);  
    if (firstField) {           
      setTimeout(() => firstField.focus = true, 300);
    } else {
      console.log('No valid field found to set focus.');
    }
  }

  findFirstFieldWithoutGroup(fields: FormlyFieldConfig[]): FormlyFieldConfig | null {
    for (const field of fields) {
      if (!field.fieldGroup && field.key) {
        return field; // Return the first field without a fieldGroup
      }
      if (field.fieldGroup) {
        const foundField = this.findFirstFieldWithoutGroup(field.fieldGroup);
        if (foundField) {
          return foundField; // Return the found field if it exists
        }
      }
    }
    return null; // Return null if no valid field is found
  } 
  ...
  setStep(index){
    ...
    this.selectActiveStep();   
    this.setFocusOnFirstField(index);
