...
export class ExternalTypeComponent extends FieldType implements OnInit, OnDestroy {
  ...
  setDescription(data: any) {
    if (typeof this.field.props.descriptionFunc === 'function'){
      const paramCount = this.field.props.descriptionFunc.length; // Get the number of parameters

      // Prepare parameters to call the function
      const params = [data];
  
      // If the function expects more than one parameter, add the second parameter
      if (paramCount > 1) {
          params.push(this.field);
      }

      this.extDescription = this.field.props.descriptionFunc(...params);   
    } else if (data){
      //il parametro decriptionProp contiene il nome della proprità che contiene la descrizione
